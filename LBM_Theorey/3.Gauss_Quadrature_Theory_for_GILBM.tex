...
\textbf{Step 5 (Lagrange closure).} Since $r\in\PP_{n-1}$, the $n$-point
Lagrange interpolant of $r$ at the nodes equals $r$ exactly, so
\[
  \I[r] \;=\; \int_I\!\Bigl[\sum_{i=1}^n r(\xi_i)\,\ell_i^{(n)}(x)\Bigr]
                 w(x)\,dx
        \;=\; \sum_{i=1}^n w_i\,r(\xi_i)
        \;=\; \sum_{i=1}^n w_i\,f(\xi_i)
        \;=\; \Q_n[f].
\]
Combining Steps~3--5 gives $\I[f]=\Q_n[f]$.
...
\subsection{Closed forms for the weights}

The Christoffel--Darboux identity \cite[Thm.~3.2.2]{Szego1975} yields the
equivalent representations
\begin{equation}
  w_i \;=\; \frac{h_{n-1}}{\phi_{n-1}(\xi_i)\,\phi_n'(\xi_i)}
       \;=\; -\,\frac{h_n}{\phi_n'(\xi_i)\,\phi_{n+1}(\xi_i)},
  \label{eq:weights-cd}
\end{equation}
from which the concrete formulas for Legendre and Hermite follow.
...
Nodes: $\xi_i^{\GL}$ are the roots of $P_n$. Weights, by specialization of
\eqref{eq:weights-cd}:
\begin{equation}
...
Nodes: $\xi_i^{\GH}$ are the roots of $\HE_n$. Weights:
\begin{equation}
